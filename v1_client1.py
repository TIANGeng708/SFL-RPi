    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                torch.save(client_fx, 't1_send1.pt')
                torch.save(labels, 't1_send2.pt')
                with open('t1_client.txt', 'wb') as file:
                ...
                while True:
                    ...
                    try:
                        print('b')
                        r2 = requests.get('http://192.168.1.107:8001/s1/t.txt', timeout=5)
                    except:
                        print('c')
                        continue
                ...
                while True:
                    ...
                    try:
                        print('f')
                        r1 = requests.get('http://192.168.1.107:8001/s1/server_send.pt', timeout=5)
                    except:
                        print('g')
                        continue
                    break
                print('h')
                with open('dfx1.pt', 'wb') as file:
                    file.write(r1.content)
                dfx = torch.load('dfx1.pt')
...
for iter in range(epochs):
    ...
    while True:
        try:
            avg = requests.get('http://192.168.1.107:8001/s1/avg_c_read.txt', timeout=5)
        except:
            continue
        if avg.content.decode() == '+':
            try:
                avg_new = requests.get('http://192.168.1.107:8001/s1/avg_new.pt', timeout=5)
