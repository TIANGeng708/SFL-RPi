    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                torch.save(client_fx, 't6_send1.pt')
                torch.save(labels, 't6_send2.pt')
                with open('t6_client.txt', 'wb') as file:
                ...
                while True:
                    ...
                    try:
                        print('b')
                        r2 = requests.get('http://192.168.1.107:8006/s6/t.txt', timeout=5)
                    except:
                        print('c')
                        continue
                ...
                while True:
                    ...
                    try:
                        print('f')
                        r1 = requests.get('http://192.168.1.107:8006/s6/server_send.pt', timeout=5)
                    except:
                        print('g')
                        continue
                    break
                print('h')
                with open('dfx6.pt', 'wb') as file:
                    file.write(r1.content)
                dfx = torch.load('dfx6.pt')
...
for iter in range(epochs):
    ...
    while True:
        try:
            avg = requests.get('http://192.168.1.107:8001/s6/avg_c_read.txt', timeout=5)
        except:
            continue
        if avg.content.decode() == '+':
            try:
                avg_new = requests.get('http://192.168.1.107:8001/s6/avg_new.pt', timeout=5)
